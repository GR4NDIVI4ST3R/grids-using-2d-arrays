...
let template = [ //* Create Custom Menu
{
  label: 'Preferences',
  submenu: [{    
    label: 'Reload',
    ...
    click: (item, focusedWindow) => {
      if (focusedWindow) {
        // On reload, close any open secondary windows
    ...
    label: 'Info',
    click() { shell.openExternal('https://youtube.com') } //! Change dis later
  }]
},
{
  label: 'Exit',
  click() { app.quit() } // Other option: win.close()
...
function createWindow () {
  ...
  var menu = Menu.buildFromTemplate(template); // End of .buildFromTemplate
